...
{
    uint32 accepted_versions[] = EXPECTED_MANGOSD_CLIENT_BUILD;
...
{
    std::cerr << "ERROR: Size of '" << filename << "' setted by format string (" << fsize << ") not equal size of C++ structure (" << rsize << ").\n";
...
inline void LoadDBC(LocalData& localeData, StoreProblemList& errlist, DBCStorage<T>& storage, const std::string& dbc_path, const std::string& filename)
{
    ...
    else
    {
        ...
        if(f)
        {
            ...
            buf << " (exist, but have " << storage.GetFieldCount() << " fields instead " << strlen(storage.GetFormat()) << ") Wrong client version DBC file?";
            errlist.push_back(dbc_filename + buf.str());
            fclose(f);
        }
        else
